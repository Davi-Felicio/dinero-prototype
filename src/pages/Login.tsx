  ...
  return (
    <div className="min-h-screen bg-background flex flex-col px-6">
      ...
      <div className="pt-20 pb-8">
        <h1 className="text-3xl font-bold tracking-tighter-custom text-foreground">
          {isLogin ? "Entrar" : "Criar conta"}
        </h1>
        <p className="text-sm text-muted-foreground mt-1">
          {isLogin ? "Seu patrimônio, sob controle cirúrgico." : "Comece a controlar suas finanças."}
        </p>
      </div>
      ...
      <div className="space-y-3 flex-1">
        ...
        {isLogin && (
          <button className="text-xs text-primary font-medium">Esqueceu a senha?</button>
        )}

        <button
          onClick={() => navigate("/")}
          className="w-full py-3.5 rounded-xl bg-primary text-primary-foreground font-semibold text-sm shadow-lg shadow-primary/20 transition-brand active:scale-[0.98] mt-4"
        >
          {isLogin ? "Entrar" : "Criar conta"}
        </button>
        ...
        {/* Google */}
        <button className="w-full py-3 rounded-xl surface-2 border border-border text-sm font-medium text-foreground flex items-center justify-center gap-2 transition-brand active:scale-[0.98]">
